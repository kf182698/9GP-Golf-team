...
## 階段 4 第十一回合實作決定（2026-09-12：發布防呆，杜絕重複發布）

- **事故**：2026-09-12 濱海場連按兩次發布，相隔 36 秒，**兩次 score.yml 都成功**
  （7/25 八里那次是三次觸發、兩次因 git push 競爭失敗才僥倖躲過）。第二次用
  **已經被扣減過的差點**重算，傷害遠不只差點多扣：洪榮杰淨桿由 97−30=67 變成
  97−27=70、黃予安由 69 變成 72，**排名整批翻轉**（淨桿冠從洪榮杰變成陳奕仲），
  獎項跟著全錯；差點再扣一次（洪榮杰 27→25、陳奕仲 20→18）。已以 revert
  第二次的確認稿與計分 commit 還原至第一次發布的正確狀態。
- **根因**：`doPublish()` 執行期間雖有 `pubBtn.disabled = true`，但
  ① 唯一的進度回饋是按鈕**下方**的 `#pubLog` 文字，手機畫面根本看不到，按鈕外觀
  幾乎沒變 → 總幹事以為沒反應；② 成功後 `finally` 又把按鈕**解鎖**，於是流程跑完
  （約 10~15 秒，含 run URL 輪詢）後再按就真的又發一次。
- **四層防護**（`admin.html`）：
  1. `publishing` 旗標在進入非同步前就擋下重複呼叫（早於 `disabled` 生效，防極快連點）。
  2. 按鈕文字改為「⏳ 發布中…請勿重複按」、轉灰，並 `scrollIntoView` 把紀錄區捲到畫面
     中央——讓「有在動」看得見，這是治本的一層。
  3. 成功後 `publishedOk = true`，`finally` 改為 `btn.disabled = publishedOk`，
     **不再解鎖**，按鈕轉綠顯示「✅ 已發布完成」；失敗才復原標籤允許重試。
     `renderStep5()` 的按鈕 markup 也讀 `publishedOk`，避免重繪時狀態被洗掉。
  4. 發布前直接抓 `golf_scores.json` 比對該日期是否已有紀錄，有就跳 `confirm()`
     說明「會重複扣減差點」並要求二次確認。**這層在重新整理頁面後仍有效**，是最後防線。
- **驗證**：Playwright 實機攔截 GitHub API 計次——已發布日期按取消時 PUT/dispatch 皆為 0；
  執行中與完成後狂點共 10 次，實際只送出 1 次 PUT 與 1 次 dispatch。

## 階段 4 第十回合實作決定（2026-07-25：首頁逐洞完整明細表）

- **golf_scores.json 新增兩個欄位**（schema 擴充，向後相容）：出賽者列加
  `逐洞`（18 格陣列）與 `每洞標準桿`（該場 18 洞 par）；請假者兩欄為 `null`。
  理由：核心原則明定「index.html 只讀 golf_scores.json」，逐洞明細所需資料
  必須寫進唯一資料源，不得另開檔案或讓前端去讀 rules.yaml。
  `每洞標準桿` 逐列重複是刻意的——扁平陣列 schema 下每列需自足，
  前端取該場任一有值的列即可。
- `publish.py::build_match_rows()` 由 `match_input` 的 `front_9`/`back_9`/`hole_pars`
  自動寫入這兩欄，**日後每場發布自動具備**，無需手動補。
- **2026-07-25 八里已回填**（取自該場確認稿，逐洞加總與各人總桿數逐一驗證吻合）；
  更早的場次無此資料，前端偵測不到 `逐洞` 就不顯示明細表，不會出錯。
- **index.html 新增「📋 逐洞完整明細」區塊**，置於最新例賽快報下方：
  - 風格與快報表格完全一致（同樣 `bg-white rounded-xl shadow` 卡片、
    `bg-primary` 標題列、`bg-gray-50 text-gray-500` 表頭、`p-2 text-sm` 儲存格）。
  - 欄位：球員｜H1–H9｜前九｜H10–H18｜後九｜總桿，共 22 欄；
    球員欄 `sticky left-0` 固定，手機橫向捲動時姓名不跑掉。
  - 逐格依相對 par 上色：博蒂以下綠、老鷹加粗、柏忌灰、雙柏忌以上紅（accent）。
  - 依總桿由低到高排序，首列淡橘底（與快報冠軍列同一視覺語彙）。
  - 標題列可點擊收合表身（▲/▼），**選定球員後自動隱藏**（表格寬且與個人檢視
    無關），符合原有「選球員 → 顯示個人儀表板」的呈現方式；最新例賽快報維持顯示。

## 階段 4 第九回合實作決定（2026-07-25：多九洞區球場 + 近洞獎可同人多得）

- **近洞獎改為計次，同一人可得多個**（bug 修正）：原本按鈕是 toggle，同一人只能有
  或沒有，但近洞獎本質是**每個 par-3 洞各發一位**，同一人本來就可能拿下多洞。
  2026-07-25 八里場有 4 個 par-3 洞（第 5、8、12、16 洞）卻只登得了 2 個近洞獎，
  就是被這個 toggle 擋住。修法：
  - admin.html 步驟⑤點名字為 **+1 累加**，旁邊出現「−」鈕可減一，超過 2 個顯示
    「×N」；上限為本場 par-3 洞數（`S.holePars` 算出，並在說明列出洞號）。
  - `publish.py::player_awards()` 由 `set` 改為計次 `dict`，2 個以上輸出
    「近洞獎×2」，獎品數量才不會在 golf_scores.json 中遺失。Eagle 獎同樣適用
    （章程本就明定一人多隻重複發放）。單一獎項字串維持原樣，**向後相容**。
  - admin.html 排名預覽同步顯示「×N」；獎項排序改以 AWARD_ORDER **前綴**比對
    （原為完全相等，帶「×2」後綴會被濾掉）。
- **支援三個以上九洞區的球場（如 A/B/C 區）**：`applyCourse()` 原本寫死
  `c.nines[0]` 與 `c.nines[1]`，三區球場只會永遠取前兩區。修法：
  - 新增 `S.nineSel`（當場選定的兩區名稱），`applyCourse()` 優先採用
    `default_combination`、否則取前兩區；新增 `applyNines()` 依選定兩區帶入
    `holePars` / `nineNames` / `coursePar`。
  - `coursePar` 一律由**選定兩區加總**求得，不再直接用 `c.par_total`——三區球場的
    `par_total` 只是預設組合的合計，換區後會不同。
  - 步驟①在球場 `nines.length > 2` 時顯示「下場區域」兩個下拉；前後半選到同一區
    時擋下「下一步」並提示。兩區球場維持原本無額外 UI。
  - rules.yaml courses 區塊附三區球場登錄範例（註解）。
- **八里球場補登 courses**：每洞 par 取自 2026-07-25 手寫成績卡
  （前九 4,4,4,4,3,5,5,3,4／後九 4,4,3,5,4,4,3,4,5，合計 72），與該場實際發布的
  `hole_pars` 逐洞比對一致。區域名稱成績卡僅標 OUT/IN，暫以前九/後九登錄待補正。
- **2026-07-25 八里例賽已發布內容全項驗算通過**：淨桿=總桿−差點全對；同分（林家進／
  陳彥宇淨桿同 66）依差點少者在前正確；幸運獎依淨桿冠 66 為雙數發給名次 2/4/6/8/10
  正確；差點扣減（林家進 21→18、陳彥宇 33→30、陳翊彥 14 不變）符合區間表；
  Eagle 無、名冊 15 人無遺漏（出賽 11 + 請假 4）。**總桿冠為林家進而非總桿最低的
  陳翊彥（81），係因陳翊彥本會期已於 2025-10-18 領過，觸發年度限領順延**，屬正確行為。

## 階段 4 第八回合實作決定（2026-07-25：金標 fixture 與活名冊脫鉤 + 2026-05-22 補正）

- **金標 fixture 改讀凍結規則快照**（重要架構修正）：`test_scoring.py` 與
  `test_publish.py` 原本直接 `load_rules(rules.yaml)`，但名冊差點**每場例賽都會被
  自動扣減更新**（`publish.py` 寫回 rules.yaml）。這代表任何一次正常的差點調整都會
  改動 fixture 中球員（如曹勇良）的淨桿，打壞金標比對——而 `score.yml` 發布前會跑
  pytest 當守門，測試一壞則**所有後續一鍵發布都會被擋下**。修法：新增
  `tests/fixtures/2025-06-21_taoyuan_rules.yaml`（fixture 驗證當下 rules.yaml 的凍結
  副本，含當期差點），兩個測試檔改讀此快照。只有「計積規則本身」變動（獎項/互斥/
  區間邏輯）時才需同步更新快照；名冊差點更新不再誤傷測試。
- **2026-05-22 礁溪例賽補正**（總幹事裁定，直接記入）：原種子資料此場僅 10 列、只登
  近洞獎、且陳奕仲差點為 19.6（小數，與名冊 20 不符）。依總幹事指示以標準發布路徑
  （引擎 + `build_match_rows`）重建為完整 16 列（滿名冊 + 請假 + 來賓黃心瀅）：
  - **總桿冠強制記楊嘉一**：引擎自然計算為陳淂笙（總桿 89 最低），但總幹事裁定本場
    總桿冠改記楊嘉一（覆寫 `result["awards"]["gross_champion"]["winner"]`）。
  - **近洞獎本場不計**（`manual_awards.near_pin = None`）；淨桿冠曹勇良、幸運獎依單雙
    判定為淨桿排名 3/5/7/9（林家進、陳淂笙、楊嘉一、王建亞）照常。
  - 差點扣減寫回 rules.yaml：曹勇良 36→32、曾吟芳 36→33、林家進 22→21。
  - 陳奕仲差點由 19.6 修正回名冊值 20（淨桿 75）。
- **礁溪高爾夫球場補登 courses**：naming 短名一致（「2026年05月例賽（礁溪）」），
  每洞 par 取自成績卡；前後九區域名稱成績卡未標，暫以前九/後九登錄待補正。

## 階段 4 第七回合實作決定（2026-07-25：步驟⑤排名/獎項發布前預覽）

- **問題**：步驟⑤只對「阻斷性同分」（淨桿與差點皆同）出裁定卡；最常見的
  「淨桿同、差點不同 → 差點少者優先」是引擎默默排的，總幹事在發布前完全
  看不到最終順序，也看不到得獎名單——第一次看到結果已是 Action summary，
  而那時 golf_scores.json 已寫入、差點已扣減寫回 rules.yaml，木已成舟。
  重發布同日期雖會整批替換成績列（冪等），但差點已被扣過，直接重發會
  **重複扣減差點**，所以「發布後不可調整」在操作上是真的。
- **解法：admin.html 步驟⑤新增「🏁 排名與獎項預覽」卡**（本地即時試算）：
  - 邏輯鏡射 `scoring.py::rank_members` 排序鍵 `(net, handicap, manual_tie_order)`；
    參數（獎項設定、exclusivity、award_period）全取自已載入的 rules.yaml，
    不另行 hardcode——預覽標示「正式結果以計分引擎為準」，引擎仍是權威。
  - 表格含名次/總桿/差點/淨桿/獎項欄；同淨桿列附說明（差點少者在前／由裁定排序）。
  - 總桿冠年度限領順延：fetch 同源 golf_scores.json + meta.award_period 比對
    會期內已領名單，順延時顯示說明；歷史載入前顯示載入中，載入後自動重繪。
  - 幸運獎單雙判定與淨桿冠互斥、Eagle 逐洞判定、近洞獎選擇皆入預覽。
  - 阻斷性同分未裁定時，預覽卡顯示提示導向上方裁定卡，裁定後即出完整預覽。
  - 驗證：以桃園金標 fixture 交叉比對 JS 預覽與 Python 引擎輸出，排名（含兩組
    差點裁決同分）、幸運獎名單、淨桿冠全數一致。
- **發布不可逆的護欄**：發布流程說明加註警告——發布後發現錯誤不得直接重發
  （差點會重複扣減），須先還原上次發布（git revert golf_scores.json 與
  rules.yaml）再修正重發。

## 階段 4 第六回合實作決定（2026-07-24：幸運分享獎排除淨桿冠軍 + 發布體驗調整）

- **幸運分享獎不再與淨桿冠軍重複發放**：`rules.yaml` 的 `exclusivity.net_and_lucky_share`
  過去雖然寫著 `"both"`，但 `scoring.py` 從來沒有真的讀取這個欄位——「排名 1 恰好落在
  單雙名次內時也一併得幸運分享獎」只是巧合的預設行為，不是被規則檔控制的。這違反
  「rules.yaml 是唯一規則來源」的原則。總幹事裁定：淨桿冠軍已經得過淨桿冠獎，不該
  再拿一次幸運分享獎，所以把這個開關真正接上 `compute_lucky_share()`：預設值改為
  `"exclude"`，排名 1 即使單雿判定命中也會從得獎名單移除（不遞補，名次序列不變，
  單純少一位得獎者）；改回 `"both"` 則恢復原本可重複得獎的行為。
  金標 fixture（`tests/fixtures/2025-06-21_taoyuan_expected.json`）與對應測試已同步
  更新（冠軍 71 單數 → 舊預期 1,3,5,7 得獎，新預期排除排名 1 後為 3,5,7）。
- **`publish.py` 發布明細加入原始差點**：`### 淨桿排名` 每列在「總桿」後、「淨桿」前
  插入「差點 {原始差點}」，方便總幹事人工核對 總桿−差點=淨桿，該差點是本場計算「調整前」
  的名冊現值（差點調整寫回 rules.yaml 在計分之後才發生，順序上不會互相污染）。
- **admin.html 發布完成後改為可點擊連結**：原本只是純文字貼出
  `https://github.com/{repo}/actions`（workflow 總覽頁，還要自己找是哪一次執行）。
  `workflow_dispatch` API 本身不回傳觸發的 run id，改為 dispatch 後以觸發時間戳記
  比對 `GET .../actions/workflows/score.yml/runs` 清單最新一筆的 `created_at`，
  找到就直接連到該次執行的 `html_url`（最多重試 4 次、每次間隔 1.5 秒等 API 更新）；
  逾時仍找不到則退回 workflow 總覽頁，不阻擋發布流程本身視為成功。

## 階段 4 第五回合實作決定（2026-07-24：修正名字候選選單無法正確選取）

- **問題**：`CLAUDE.md` 定義的 `name_candidates` schema 是「已排版好的完整字串」
  （如 `"王建亞 95%"`、`"新增來賓：王建亚"`），但 `admin.html::applyDraft()`
  的 `uncertain` 分支誤把整個字串當成純姓名處理，還另外用不存在的
  `p.name_similarities` 欄位重新算一次百分比疊加在後面——選單顯示變成
  「陳淂笙 67% 60%」雙重百分比，且總幹事點選候選人後，`S.players[i].name`
  會被寫入整串「陳淂笙 67%」而不是乾淨的「陳淂笙」，該球員自此對不上名冊、
  無法正確排名／領獎，形同這個候選項「選不了」。
- **解法**：`applyDraft()` 改為解析 `name_candidates` 每個字串——
  `新增來賓：` 前綴代表新增來賓選項（去掉前綴取得原始姓名）；否則以
  `/^(.+?)\s+(\d+)%$/` 拆出乾淨姓名與相似度，`label` 保留原始字串供顯示、
  `name` 只放乾淨姓名供選中時寫入 `S.players[i].name`。下拉選單渲染改直接用
  預先算好的 `c.label`，不再於渲染時重新拼接百分比。
...
- 幸運分享獎單雙判定（淨桿冠軍 71 為單數 → 排名 1,3,5,7 落在名次內，但排名 1
  已得淨桿冠軍不重複得幸運分享獎，實際得獎 3,5,7）
- Eagle 為空的情境
- 球場 par 73 ≠ 差點基準 72
